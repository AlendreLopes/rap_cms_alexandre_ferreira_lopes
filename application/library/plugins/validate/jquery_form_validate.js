...
jQuery.iFormValidate = {
			...
			function()
			{
				...
				$(this).submit(function()
				{
					...
					if(options.ajax)
					{
						...
						// I change this line to load file in a div especific
						// $(this).parent('div').fadeOut("slow", function(){
						$(options.divTarget).fadeOut("slow", function(){
							$(this).load(options.phpFile, data, function(){$(this).fadeIn("slow");});
						});
	...
	validateForm : function($inputs)
	{
		...
		if($inputs.filter(".is_required").hasClass("invalid"))
		{
			isValid = false;
		}
		return isValid;
	...
	validate : function()
	{
		...
		else if($(this).hasClass('vphone'))
		{
			...
			if((!Regex.test($val))&&(!Regex2.text($val)))
			{
				isValid = false;
			}
		...
		else if($(this).hasClass('vpasswordconfirm'))
		{
			...
			while($el.attr("tagName").toLowerCase() != "form")
			{
				$el = $el.parent();
			}
