<?php
$Proxima = $_GET['indice'];
$texto_1 = "SELECT count(*) AS Total FROM protocolos, laudos_#A# WHERE protocolo_id = _laudo AND _data BETWEEN CURDATE() - INTERVAL 4 MONTH AND CURDATE()";
...
$texto_2 = "SELECT
 ...
 _clinica_titulo,
 _concluido
FROM protocolos, laudos_#A#  WHERE protocolo_id = _laudo AND _data BETWEEN CURDATE() - INTERVAL 4 MONTH AND CURDATE() ORDER BY protocolo_id DESC LIMIT $Limitar";
				...
				while ($Listar = mysql_fetch_array($Laudo)) {
					...
					<tr <?php echo $y; ?>>
						<td align="center" title="Editar | Visualizar Protocolo: <?php echo $Listar['_protocolo']; ?>">
							<a href="<?php echo $url_n; ?>"><?php echo $Listar['_protocolo']; ?></a>
						</td>
