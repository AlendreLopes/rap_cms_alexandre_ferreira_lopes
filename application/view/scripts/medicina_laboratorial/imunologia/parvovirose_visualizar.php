...
$Exames  = $SQL -> Consultar("SELECT * FROM protocolos, laudos_ml_i_parvovirose WHERE protocolo_id = '$indice' AND _laudo = '$indice'");
...
$Otimizar = $SQL -> Atualizar("OPTIMIZE TABLE protocolos, laudos_ml_i_parvovirose");
...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			var t_t   = "laudos_ml_i_parvovirose";
			var c_i   = "_id";
			var c_i_v = $("#_id").attr("indice");
			
			$("#_parvovirus").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_parvovirus"}
				}
			);
			
			$("#_coronavirus").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					data       :"{'Positivo':'Positivo','Negativo':'Negativo','selected':'Selecionar'}",
					type       :'select',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_coronavirus"}
				}
			);
			
			$("#_resultado").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					data       :"{'Positivo':'Positivo','Negativo':'Negativo','selected':'Selecionar'}",
					type       :'select',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_resultado"}
				}
			);
...
<fieldset>
	...
	<legend align="center">Laudo Parvovirose</legend>
	
	<span id="_id" indice="<?php echo $Exame[_id];?>"></span>
	
    <label>
    
	<strong>Parvov&iacute;rus</strong>
    
	<span id="_parvovirus"><?php echo $Exame[_parvovirus];?></span>
	
    </label>
	
    <label>
    
	<strong>Coronav&iacute;rus</strong>
    
	<span id="_coronavirus"><?php echo $Exame[_coronavirus];?></span>
	...
	<span id="_resultado"><?php // echo $Exame[_resultado];?></span>
