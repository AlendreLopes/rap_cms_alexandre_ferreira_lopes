...
$Exames  = $SQL -> Consultar("SELECT * FROM protocolos, laudos_ml_i_dirofilariose WHERE protocolo_id = '$indice' AND _laudo = '$indice'");
...
$Otimizar = $SQL -> Atualizar("OPTIMIZE TABLE protocolos, laudos_ml_i_dirofilariose");
...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			var t_t   = "laudos_ml_i_dirofilariose";
			var c_i   = "_id";
			...
			$("#dirofilaria").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_dirofilaria"}
				}
			);
			$("#ehrlichia").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					data       :"{'Positivo':'Positivo','Negativo':'Negativo','selected':'Selecionar'}",
					type       :'select',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_ehrlichia"}
				}
			);
			$("#borrelia").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					data       :"{'Positivo':'Positivo','Negativo':'Negativo','selected':'Selecionar'}",
					type       :'select',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_borrelia"}
				}
			);
			$("#anaplasma").editable("/application/controller/plugins/indexEditar.php",
				{
					indicator  :'Salvando',
					data       :"{'Positivo':'Positivo','Negativo':'Negativo','selected':'Selecionar'}",
					type       :'select',
					...
					cancel     :'Cancelar',
					width      :100,
					submitdata :{t_t:t_t,c_i:c_i,c_i_v:c_i_v,c_t:"_anaplasma"}
				}
			);
...
<fieldset>
	...
	<legend align="center">Laudo Dirofilariose</legend>
	
	<span id="_id" indice="<?php echo $Exame[_id];?>"></span>
	
    <label>
    
	<strong>Dirofilaria</strong>
    
	<span id="dirofilaria"><?php echo $Exame[_dirofilaria];?></span>
	...
	<span id="ehrlichia"><?php echo $Exame[_ehrlichia];?></span>
	
    </label>
    
    <label>
    
	<strong>Borrelia</strong>
    
	<span id="borrelia"><?php echo $Exame[_borrelia];?></span>
	...
	<span id="anaplasma"><?php echo $Exame[_anaplasma];?></span>
