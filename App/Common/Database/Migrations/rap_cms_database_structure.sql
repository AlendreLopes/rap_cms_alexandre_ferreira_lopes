...
CREATE TABLE `users` (
  ...
  `profile_photo_path`varchar(2048) COLLATE utf8mb4_unicode_ci NOT NULL,
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `email_UNIQUE` (`email`),
...
CREATE TABLE `orders_items` (
  ...
  `created_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_orders_items_users_idx` (`user_id`),
  KEY `fk_orders_items_orders_idx` (`order_id`),
  ...
  CONSTRAINT `fk_orders_items_users` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */
;
